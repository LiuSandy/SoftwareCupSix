...
def serialize_file(sale_name,file_name):
    ...
    # 写入文件
    result.to_csv('check_base_algorithm2.csv',columns=columns,index=False)
...
if __name__ == "__main__":
    ...
    relation = Relation()
    # 计算代理人的销售信息
    sale_sum = set.reader_file("base.csv")
    sale_name = bs.count_cnt_round(sale_sum,"sale_num.csv")
    print(sale_name)
    #计算代理人之间的关系信息
    df = format_file("every_sale.csv")
    ...
    # 将列表内的文件合并写入文件并序列化文件
    file_name = set.write_csv_data(pd.concat(frames),'base_algorithm1.csv')
